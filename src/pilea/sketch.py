...
def sketch(file, folder, k, s, w):
    ...
    kcnt = Counter()
    for cidx, record in enumerate(parse_fastx_file(file)):
        seq = record.seq
        ...
        for i in range(len(seq) - k + 1):
            if (key := hash64(seq[i:i + k])) < maxhash:
                kcnt[key] += 1
                kctg[(cidx, i // w)].append((key, scan(i + k // 2, gc_prefix)))

    wins = 0
    kdup = {key for key, val in kcnt.items() if val > 1}
    with open(f'{folder}/{idx}.pdb', 'wb') as f:
        for widx, (_, kpos) in enumerate(kctg.items()):
            if (sins := sum(not key in kdup for key, _ in kpos)) > 1:
                wins += 1
    ...
    return idx, (name, cidx + 1, len(kcnt), wins)
